...
export class CarsRoute {
  ...
  constructor() {
    ...
    this.router.delete(':id/:imageUrl/image/delete', this.deleteCarImage.bind(this));
    this.carsService = new CarsService();
  }

  async deleteCarImage(request: express.Request, res: express.Response) {
    try {
      const data = await this.carsService.deleteImage(request.params.id, request.params.imageUrl);
      res.json(data);
    } catch (error) {
      console.log(error);
